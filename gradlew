...
#   * $APP_HOME/gradle/wrapper/gradle-wrapper.properties is used as-is
#     (any properties set by the user override the defaults)
set -- \
...
if ! "$cygwin" && ! "$msys" && ! "$nonstop" ; then
    case $( set -- ...; set -- $( ulimit -c 0 2>/dev/null ); echo "$1" ;) in
      0) ;;
      *) MAX_FD=65536 ;;
    esac
fi
